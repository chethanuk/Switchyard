...
| `response_format_type` | No | `json_schema` | Structured-output mode for the judge in every mode. Use `json_object` when the provider does not support JSON Schema; Switchyard adds the schema to the prompt and validates the verdict locally. In custom mode, the configured `response_schema` is the source of truth, not a copy written into the prompt. |
